arraysAnswers = {
  ...
  sum: function sum(arr) {
    return arr.reduce((prevValue, currentItem) => prevValue + currentItem, 0);
  },
  ...
  /**
   * Change current arr, excluding item 
   * 
   * @param {Number[]} arr - An array of numbers
   * @param {Number} item - A number to be excluded from the new array
   * @returns {Number[]} The same array containing all numbers from arr except item.
   */
  removeWithoutCopy: function remove(arr, item) {
  ...
  count: function count(arr, item) {
    const result = arr.reduce((prevValue, currentItem) => {
      if (currentItem === item) {
        return prevValue + 1;
      }
  ...
  duplicates: function duplicates(arr) {
    ...
    arr.forEach((current, index) => {
      if (arr.indexOf(current, index + 1) !== -1) {
        duplicateValues.add(current);
      }
    });
